...
pretrain_epochs = 100
finetune_epochs = 50
label_fraction = 0.01  # 0.01 (1%), 0.1 (10%), 0.5 (50%)
...
def apply_wavelet_LL(image_tensor, level, wavelet='sym4'):
    ...
    LL = coeffs[0]
    LL_resized = torch.tensor(LL).permute(2, 0, 1).float()
...
class WaveletCurriculumDataset(Dataset):
    def __init__(self, base_dataset, wavelet='sym4', transform=None, max_epoch=50):
        ...
        self.transform = transform
        self.curriculum_stages = [3, 2, 1, 0]  # LL3 → LL2 → LL1 → original
    ...
    def __getitem__(self, idx):
        ...
        if self.current_epoch < self.max_epoch:
            stage_idx = self.current_epoch * len(self.curriculum_stages) // self.max_epoch
            stage = self.curriculum_stages[stage_idx]
        else:
            stage = 0
        if stage > 0:
            img = apply_wavelet_LL(img, level=stage, wavelet=self.wavelet)
        if self.transform:
...
# --- Dataset pour fine-tuning avec échantillonnage partiel ---
...
for epoch in range(finetune_epochs):
    curriculum_dataset.set_epoch(epoch)
    ...
    for x, y in trainloader:
        ...
        total += y.size(0)
    acc = correct / total
    print(f"[FineTune Epoch {epoch+1}] Loss: {total_loss:.4f}, Accuracy: {acc:.4f}")
...
pretrain_epochs = 100
finetune_epochs = 50
label_fraction = 0.1  # 0.01 (1%), 0.1 (10%), 0.5 (50%)
...
class WaveletCurriculumDataset(Dataset):
    def __init__(self, base_dataset, wavelet='db4', transform=None, max_epoch=50):
        self.base_dataset = base_dataset
        self.wavelet = wavelet
        self.transform = transform
        self.curriculum_stages = [3, 2, 1, 0]  # LL3 → LL2 → LL1 → original
...
trainset = datasets.CIFAR10(root='./data', train=True, transform=transforms.ToTensor())
subset_train = get_subset_per_class(trainset, label_fraction)
curriculum_dataset = WaveletCurriculumDataset(subset_train, wavelet='db4', transform=linear_transform, max_epoch=finetune_epochs)
trainloader = DataLoader(curriculum_dataset, batch_size=batch_size, shuffle=True)
